...
CREATE TABLE `user` (
    ...
    PRIMARY KEY (`user_id`),
    FOREIGN KEY (`branch_id`) REFERENCES `branch` (`branch_id`) ON UPDATE CASCADE ON DELETE SET NULL
);
...
CREATE TABLE `branch_manager` (
    ...
    FOREIGN KEY (`manager_id`) REFERENCES `user` (`user_id`) ON UPDATE CASCADE ON DELETE RESTRICT
);
...
CREATE TABLE `staff` (
    ...
    FOREIGN KEY (`staff_id`) REFERENCES `user` (`user_id`) ON UPDATE CASCADE ON DELETE RESTRICT
);
...
CREATE TABLE `insurance_claim` (
    ...
    FOREIGN KEY (`service_code`) REFERENCES `treatment_catelogue` (`service_code`)
);
...
SELECT p.patient_id, p.name AS patient_name, bi.net_amount, bi.remaining_payment_amount
FROM
    billing_invoice bi
    JOIN appointment a ON bi.appointment_id = a.appointment_id
    JOIN patient p ON a.patient_id = p.patient_id
WHERE
    bi.remaining_payment_amount > 0
...
SELECT
    ...
    SUM(
        CASE
            WHEN bi.claim_id IS NOT NULL THEN bi.net_amount
            ELSE 0
        END
    ) AS insurance_covered_amount,
    SUM(
        CASE
            WHEN bi.claim_id IS NULL THEN bi.net_amount
            ELSE 0
        END
